...
def handle_answer(event_body, user):
    ...
    # --- Step 2: Retroactively "top up" scores for any section where the threshold was met
    for section_id, section_config in QUESTIONS_CONFIG.get('sections', {}).items():
        ...
        # Check if the user has answered any of the trigger questions for this section
        if any(q in all_answers for q in trigger_questions):
            ...
            if (current_section_score / max_score_so_far) >= SCORING_THRESHOLD:
                print(f"Topping up score for already completed section: {section_id}")
                user_state['section_scores'][section_id] = section_config.get('total_points', current_section_score)

    # --- Step 3: Determine the next question based on the last answer
    ...
    if last_question_data:
        ...
        if last_section_config and question_id in last_section_config.get('adaptive_trigger_questions', []):
            ...
            if last_section_score == last_section_config.get('total_points'): # Check if it was topped up
    ...
    # For logged-in users, save the complete state to DynamoDB
    if user:
...
def lambda_handler(event, context):
    ...
    # --- THIS IS THE NEW DYNAMIC CORS LOGIC ---
    # Define your allowed origins. For best practice, load this from an environment variable.
    allowed_origins = [
        "http://localhost:8080",
        "https://www.your-app-domain.com" # Add your production domain here
    ]
