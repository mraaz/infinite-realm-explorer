...
const CategoryProgress: React.FC<CategoryProgressProps> = ({ categories }) => {
  ...
  // New state to determine if we are on a mobile screen size
  ...
  const toggleExpansion = () => {
    // This function will only be called if onClick is active
  ...
  return (
    <div
      // Conditionally apply onClick handler based on screen size
      onClick={isMobileScreen ? toggleExpansion : undefined}
      className="w-full max-w-5xl mx-auto mb-6 md:mb-10 overflow-hidden" // Removed cursor-pointer from here
    >
      {/* Conditionally apply cursor-pointer for mobile only */}
      <div
        className={`${
