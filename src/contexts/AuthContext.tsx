    ...
    case "SET_USER_DATA":
      return {
        ...
        completedFutureQuestionnaire:
          action.payload.completedFutureQuestionnaire,
      };
    case "LOGOUT":
      return {
        user: null,
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const updateTokenCacheAtomically = useCallback(
    (token: string, decodedUser: User) => {
      if (tokenCacheRef) {
  ...
  const updateDataCacheAtomically = useCallback(
    (data: any) => {
      if (dataCacheRef) {
        const now = Date.now();
  ...
  const validateAndDecodeToken = useCallback(
    (token: string): User | null => {
      ...
      if (
        ...
        Date.now() < tokenCacheRef.expiry - 60000
      ) {
        // 1 min buffer
        performanceRef.cacheHits++;
  ...
  const fetchUserDataCached = useCallback(
    async (token: string): Promise<void> => {
      const now = Date.now();
      const CACHE_TTL = 5 * 60 * 1000; // 5 minutes

      // Check cache validity
      if (dataCacheRef && now - dataCacheRef.timestamp < dataCacheRef.ttl) {
        performanceRef.cacheHits++;
  ...
  const fetchAndSetAuthStatus = useCallback(
    async (token: string) => {
      try {
  ...
  const fetchUserDataWithDelay = useCallback(
    async (token: string, retryCount = 0) => {
      ...
      try {
        ...
        console.warn(
          `[Auth] Failed to fetch user data (attempt ${retryCount + 1}):`,
          error
        );
  ...
  const login = useCallback(
    async (newToken: string) => {
      try {
        ...
        dispatch({
          type: "SET_USER",
          payload: { user: decodedUser, token: newToken },
        });
    ...
    if (process.env.NODE_ENV === "development") {
      const logPerformance = () => {
        console.log("[Auth Performance]", {
          ...
          cacheHitRate:
            (performanceRef.cacheHits /
              (performanceRef.cacheHits + performanceRef.cacheMisses)) *
            100,
        });
      };
